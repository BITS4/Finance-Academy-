        ...
        <View style={s.statsRow}>
          ...
          <View style={s.overallBarWrap}>
            <View style={s.overallBar}>
              <View style={[s.overallFill, { width: `${allTopics.length ? Math.round((passed / allTopics.length) * 100) : 0}%` }]} />
            </View>
            <Text style={s.overallPct}>{allTopics.length ? Math.round((passed / allTopics.length) * 100) : 0}%</Text>
          </View>
        </View>
      </View>
                ...
                <View style={s.sectionMeta}>
                  <View style={s.sectionTopRow}>
                    ...
                    {sectionPassed === section.topics.length && (
                      <View style={s.doneBadge}><Text style={s.doneBadgeText}>✓ Сдан</Text></View>
                    )}
                  </View>
                  ...
                  <View style={s.sectionBarRow}>
                    ...
                    <Text style={[s.sectionBarLabel, { color: section.color }]}>{sectionPassed}/{section.topics.length}</Text>
                  </View>
                </View>
              ...
              {isOpen && (
                <View style={s.topicsWrap}>
                  {section.topics.map((topic, tIdx) => {
                    ...
                    if (status === 'completed') { statusIcon = '✅'; statusColor = T.green; statusText = `Сдано · ${p.examScore}%`; }
                    else if (status === 'inprogress') { statusIcon = '📖'; statusColor = T.gold; statusText = 'В процессе'; }
                    else if (!locked) { statusIcon = '🔓'; statusColor = T.text; statusText = 'Доступно'; }
                    ...
                    return (
                      <View key={topic.id} style={s.topicRow}>
                        ...
                        <View style={s.tree}>
                          ...
                          <View style={[s.treeDot, {
                            backgroundColor:
                              status === 'completed' ? T.green :
                              status === 'inprogress' ? T.gold :
                              locked ? T.border : section.color,
                          }]}>
                            <Text style={s.treeDotText}>{status === 'completed' ? '✓' : tIdx + 1}</Text>
                          </View>
                        </View>
                        ...
                        >
                          ...
                          <View style={s.topicTop}>
                            ...
                            <View style={s.topicInfo}>
                              <Text style={[s.topicTitle, locked && { color: T.faint }]} numberOfLines={2}>{topic.title}</Text>
                              <Text style={[s.topicStatusText, { color: statusColor }]}>{statusIcon} {statusText}</Text>
                            </View>
                          </View>
                          ...
                          <View style={s.topicStats}>
                            <View style={[s.statChip, { backgroundColor: section.colorBg }]}>
                              <Text style={[s.statChipText, { color: section.color }]}>{topic.questions.length} вопросов</Text>
                            </View>
                            ...
                            <View style={[s.statChip, { backgroundColor: T.goldBg }]}>
                              <Text style={[s.statChipText, { color: T.gold }]}>нужно {passScore} б.</Text>
                            </View>
                          </View>
                            ...
                            {[1, 2, 3].map(d => {
                              ...
                              return (
                                <View key={d} style={s.diffChip}>
                                  ...
                                  <Text style={s.diffText}>{cnt} {labels[d]}</Text>
                                </View>
                              );
                            })}
                          </View>
                          ...
                          {locked && (
                            <Text style={s.lockMsg}>🔒 Сдайте: «{section.topics[tIdx - 1]?.title}»</Text>
                          )}
                        </TouchableOpacity>
                      </View>
                    );
...
const s = StyleSheet.create({
  ...
  overallBar: { flex: 1, height: 6, backgroundColor: T.border, borderRadius: 3, overflow: 'hidden' },
  ...
  sectionIconBox: { width: 46, height: 46, borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
  ...
  doneBadge: { backgroundColor: T.greenBg, paddingHorizontal: 7, paddingVertical: 2, borderRadius: 6 },
  ...
  treeDot: { width: 22, height: 22, borderRadius: 11, alignItems: 'center', justifyContent: 'center', marginTop: 14, zIndex: 1 },
  treeDotText: { color: '#fff', fontSize: 9, fontWeight: '800' },

  topicCard: {
    flex: 1, backgroundColor: T.surface, borderRadius: 12,
    padding: 13, borderWidth: 1, borderColor: T.border,
  },
  ...
  scoreBar: { flex: 1, height: 5, backgroundColor: T.border, borderRadius: 3, overflow: 'visible', position: 'relative' },
  scoreBarFill: { height: '100%', borderRadius: 3 },
  scoreLine: { position: 'absolute', left: '80%', top: -2, width: 2, height: 9, backgroundColor: T.gold },
