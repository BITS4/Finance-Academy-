      ...
      <Modal visible animationType="slide" onRequestClose={onClose}>
        <ScrollView style={s.container} contentContainerStyle={s.content}>
          <View style={[s.summaryCard, { backgroundColor: courseBg }]}>
            ...
            <Text style={[s.summaryScore, { color: courseColor }]}>
              {score}/{questions.length}
            </Text>
            <Text style={s.summaryLabel}>
              {pct >= 80 ? 'Отлично!' : pct >= 60 ? 'Хороший результат' : 'Нужна практика'}
            </Text>
          </View>
          ...
          <TouchableOpacity
            style={[s.doneBtn, { backgroundColor: courseColor }]}
            onPress={() => onComplete(pct)}
          >
      ...
      <View style={s.container}>
        ...
        <View style={s.progressBar}>
          <View
            style={[
              s.progressFill,
              { width: `${(cur / questions.length) * 100}%`, backgroundColor: courseColor },
            ]}
          />
        </View>
