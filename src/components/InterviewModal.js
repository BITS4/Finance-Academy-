    ...
    return (
      <Modal visible animationType="slide" onRequestClose={onClose}>
        <ScrollView style={s.container} contentContainerStyle={s.content}>
          {/* Source badge */}
          <View style={[s.sourceBadge, { backgroundColor: level.colorBg, borderColor: level.color }]}>
          ...
          <TouchableOpacity style={[s.startBtn, { backgroundColor: level.color }]} onPress={startThinking} activeOpacity={0.8}>
    ...
    return (
      <Modal visible animationType="none" onRequestClose={onClose}>
        <View style={s.container}>
          ...
          <ScrollView contentContainerStyle={s.thinkingContent} showsVerticalScrollIndicator={false}>
            {/* Question reminder */}
            ...
            <View style={s.hintsSection}>
              ...
              {q.hints.map((hint, i) => {
                ...
                return (
                  <TouchableOpacity
                    key={i}
                    style={[s.hintCard, shown ? { borderColor: level.color, backgroundColor: level.colorBg } : s.hintCardHidden]}
                    onPress={() => { if (!shown) setHintsShown(i + 1); }}
            ...
            {q.correctHypotheses && (
              <View style={s.hypothesesSection}>
                ...
                {q.correctHypotheses.map(h => (
                  <TouchableOpacity
                    key={h.id}
                    style={[s.hypothesisCard, hypothesesSelected.includes(h.id) && { borderColor: level.color, backgroundColor: level.colorBg }]}
                    onPress={() => toggleHypothesis(h.id)}
                    activeOpacity={0.8}
                  >
                    <View style={[s.hypothesisCheck, hypothesesSelected.includes(h.id) && { backgroundColor: level.color }]}>
                      {hypothesesSelected.includes(h.id) && <Text style={s.hypothesisCheckMark}>✓</Text>}
                    </View>
                    <Text style={[s.hypothesisText, hypothesesSelected.includes(h.id) && { color: level.color }]}>{h.title}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            )}

            <TouchableOpacity style={[s.revealBtn, { backgroundColor: level.color }]} onPress={showReveal} activeOpacity={0.8}>
      ...
      <Modal visible animationType="slide" onRequestClose={onClose}>
        <ScrollView style={s.container} contentContainerStyle={s.content}>
          ...
          {q.correctHypotheses && hypothesesSelected.length > 0 && (
            <View style={[s.scoreCard, { backgroundColor: correctSelected >= 2 ? T.greenBg : T.redBg }]}>
              <Text style={[s.scoreEmoji]}>{correctSelected >= 3 ? '🏆' : correctSelected >= 2 ? '✅' : '📚'}</Text>
          ...
          {q.correctHypotheses && (
            <View style={s.hypoReview}>
              ...
              {q.correctHypotheses.map(h => {
                ...
                return (
                  <View key={h.id} style={[s.hypoReviewCard, wasSelected ? s.hypoCorrect : s.hypoMissed]}>
                    <View style={s.hypoReviewTop}>
                      <Text style={[s.hypoReviewIcon]}>{wasSelected ? '✅' : '📝'}</Text>
                      <Text style={[s.hypoReviewText, { color: wasSelected ? T.green : T.text }]}>{h.title}</Text>
                    </View>
          ...
          <View style={s.tipsCard}>
            ...
            <Text style={s.tipsBody}>• Начинай с фреймворка: "Я бы разбил проблему на X и Y..."\n• Используй числа и конкретные метрики\n• Показывай причинно-следственные связи\n• Финиши с чёткой рекомендацией</Text>
          </View>
          ...
          <TouchableOpacity style={s.cancelBtn} onPress={() => { setPhase('intro'); setTimer(0); setHintsShown(0); setHypothesesSelected([]); }} activeOpacity={0.7}>
...
const s = StyleSheet.create({
  ...
  sourceBadge: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 10, borderWidth: 1, alignSelf: 'flex-start', marginBottom: 8 },
  ...
  questionCard: { backgroundColor: T.surface, borderRadius: 16, padding: 16, borderWidth: 2, marginBottom: 16 },
  ...
  timerInfoItem: { flex: 1, backgroundColor: T.surface, borderRadius: 10, padding: 10, alignItems: 'center', borderWidth: 1, borderColor: T.border },
  timerInfoIcon: { fontSize: 18, marginBottom: 3 },
  timerInfoText: { color: T.sub, fontSize: 11, textAlign: 'center' },

  howToCard: { backgroundColor: T.card, borderRadius: 14, padding: 16, marginBottom: 20, borderWidth: 1, borderColor: T.border },
  ...
  // Thinking phase
  timerTop: { flexDirection: 'row', alignItems: 'baseline', justifyContent: 'center', paddingTop: 52, paddingBottom: 8 },
  ...
  questionCardSmall: { backgroundColor: T.surface, borderRadius: 12, padding: 12, borderWidth: 1, marginBottom: 16 },
  ...
  hintCard: { borderRadius: 12, padding: 12, borderWidth: 1, borderColor: T.border, marginBottom: 8 },
  ...
  hypothesisCheck: {
    width: 24, height: 24, borderRadius: 6, backgroundColor: T.card,
  ...
  modelCard: {
    backgroundColor: T.surface, borderRadius: 16, padding: 16, borderWidth: 2, marginBottom: 16,
  },
  ...
  tipsCard: { backgroundColor: T.card, borderRadius: 14, padding: 14, marginBottom: 16, borderWidth: 1, borderColor: T.border },
