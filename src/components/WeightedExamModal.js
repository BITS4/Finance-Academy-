      ...
      <Modal visible animationType="slide" onRequestClose={onClose}>
        <ScrollView style={s.container} contentContainerStyle={s.introContent}>
          <View style={[s.topicBadge, { backgroundColor: section.colorBg }]}>
            ...
            <Text style={[s.topicBadgeText, { color: section.color }]}>
              {section.title} · Раздел {section.number}
            </Text>
          </View>
          ...
          <View style={s.contentCard}>
            ...
            <Text style={s.contentText} numberOfLines={8}>
              {topic.content.replace(/#{1,3} /g, '').replace(/\*\*/g, '')}
            </Text>
          </View>

          <TouchableOpacity
            style={[s.startBtn, { backgroundColor: section.color }]}
        ...
        <ScrollView style={s.container} contentContainerStyle={s.introContent}>
          ...
          <View style={s.breakdownCard}>
            ...
            {[1, 2, 3].map((d) => {
              ...
              const earned_d = answers.filter(
                (a, i) => questions[i]?.difficulty === d && a.isCorrect,
              ).length;
              return (
                <View key={d} style={s.breakdownRow}>
                  <Text style={[s.breakdownDiff, { color: DIFF_COLOR[d] }]}>{DIFF_LABEL[d]}</Text>
                  <Text style={s.breakdownResult}>
                    {earned_d}/{qs.length} верных × {d}б = {earned_d * d}/{qs.length * d}б
                  </Text>
                </View>
              );
            })}
          ...
          {finalPassed ? (
            <TouchableOpacity
              style={[s.startBtn, { backgroundColor: T.green }]}
              onPress={() => onPass(finalPct, earned, maxScore)}
            >
    ...
    <Modal visible animationType="slide" onRequestClose={onClose}>
      <View style={s.container}>
        ...
        <View style={[s.quizHeader, { borderBottomColor: section.color + '44' }]}>
          <Text style={[s.quizTitle, { color: section.color }]}>
        ...
        <View style={s.progBar}>
          <View
            style={[
              s.progFill,
              { width: `${(cur / totalQ) * 100}%`, backgroundColor: section.color },
            ]}
          />
        </View>
          ...
          {q && (
            <View style={[s.diffBadge, { backgroundColor: DIFF_COLOR[q.difficulty] + '22' }]}>
              <Text style={[s.diffBadgeText, { color: DIFF_COLOR[q.difficulty] }]}>
                {DIFF_LABEL[q.difficulty]} · стоит {q.difficulty}{' '}
                {q.difficulty === 1 ? 'балл' : q.difficulty < 5 ? 'балла' : 'баллов'}
              </Text>
            </View>
          )}
          ...
          <View style={s.options}>
            {q?.opts.map((opt, i) => {
              ...
              if (confirmed) {
                ...
                optStyle = [
                  s.option,
                  { borderColor: section.color, backgroundColor: section.colorBg },
                ];
              }
              return (
                <TouchableOpacity
