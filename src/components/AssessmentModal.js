...
export default function AssessmentModal({ course, onClose }) {
  ...
  function next() {
    ...
    const ans = [...answers, { sel: selected, correct: questions[cur].correct, topic: questions[cur].topic }];
    setAnswers(ans);
    setSelected(null);
    if (cur < questions.length - 1) {
      ...
      const weak = [...new Set(ans.filter(a => a.sel !== a.correct).map(a => a.topic))];
      setResults({ score, total: questions.length, weak });
      setPhase('results');
    }
  }
        ...
        <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
          {phase === 'quiz' && q && (
            <View>
              <View style={s.bars}>
                {questions.map((_, i) => (
                  <View key={i} style={[s.bar, {
                    backgroundColor: i < cur ? course.color : i === cur ? course.color + '70' : T.border,
                  }]} />
                ))}
              </View>
              <Text style={s.qNum}>Вопрос {cur + 1} из {questions.length}</Text>
              <Text style={s.question}>{q.q}</Text>
              <View style={s.options}>
                {q.opts.map((opt, i) => (
                  <TouchableOpacity
                    key={i}
                    style={[s.option, selected === i && { borderColor: course.color, backgroundColor: course.colorBg }]}
                    onPress={() => setSelected(i)}
                    activeOpacity={0.7}
                  >
                    <View style={[s.dot, selected === i && { backgroundColor: course.color, borderColor: course.color }]}>
              ...
              <TouchableOpacity
                style={[s.btn, { backgroundColor: course.color }, selected === null && s.btnDisabled]}
                onPress={next}
          ...
          {phase === 'results' && results && (
            <View>
              <View style={[s.scoreCard, { backgroundColor: course.colorBg }]}>
                <Text style={s.scoreEmoji}>{results.score >= 4 ? '🎉' : results.score >= 3 ? '👍' : '📚'}</Text>
                <Text style={[s.scoreNum, { color: course.color }]}>{results.score}/{results.total}</Text>
              ...
              <TouchableOpacity style={[s.btn, { backgroundColor: course.color }]} onPress={onClose}>
...
const s = StyleSheet.create({
  ...
  weakCard: {
    backgroundColor: T.surface, borderRadius: 14, padding: 18, marginBottom: 24,
